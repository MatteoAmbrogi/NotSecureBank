...
public class ServletUtil {
    ...
    static public boolean isLoggedinAdmin(HttpServletRequest request) {
        ...
        try {
            // Check user is logged in
            String userAdmin = (String) request.getSession().getAttribute(ServletUtil.SESSION_ATTR_ADMIN_KEY);
            if (userAdmin != SESSION_ATTR_ADMIN_VALUE) {
                LOG.info("False.");
                return false;
            }
        } catch (Exception e) {
    ...
    public static boolean isPreApprovedForGoldVisa(HttpServletRequest request) {
        ...
        boolean isPreApprovedForGoldVisa = (Boolean) request.getSession().getAttribute(SESSION_ATTR_PREAPPROVEDFORGOLDVISA);
